...
我们直奔主题，从 Docker 镜像的存储入手，看看这些`镜像层文件内容`与`镜像 json 文件`分别存储于何处。（以下展示的实验环境：宿主机操作系统为 Ubuntu 14.04、Docker 版本为 1.7.1、graphdriver 类型为 aufs，仅包含 `ubuntu:14.04` 一个镜像。1.10版本以后的Docker存储发生了较大的变化，具体详情可见[展望 Docker 1.10 镜像新面貌](http://blog.daocloud.io/docker-1-10-2/)）
